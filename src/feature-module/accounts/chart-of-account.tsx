...
import { GetChartOfAccount, GetRegionChartOfAccount } from "../../store/apps/ho-coa";
import { AppDispatch, RootState } from "../../store";
import { AddChartofAccount, DeleteCOAccount, GetChartofAccountByID, UpdateCOAccount } from "../../store/apps/ho-coa";
...
const ChartOfAccount = () => {
    ...
    const hasPermission = usePermission("Chart of Accounts");
    const regionsList = useRegionsList();
    const { list, loading, regionData } = useSelector((state: RootState) => state.hoCoa);
    ...
    const [searchRegionId, setSearchRegionId] = useState<number | null>(null)
    const handleRegionCOA = async (value: string | number) => {
        const region = Number(value);
        setSearchRegionId(region)
    }
    const [regionId, setRegionId] = useState(0)
    const handleChangeRegions = async (value: string | number) => {
    ...
    useEffect(() => {
        if (searchRegionId !== null) {
            dispatch(GetRegionChartOfAccount(searchRegionId));
        } else {
            dispatch(GetChartOfAccount());
        }
    }, [dispatch, searchRegionId]);

    useEffect(() => {
        if (userLevel === 2 && userLevelId) {
            setRegionId(userLevelId);
            handleEntryLevel(4);
        }
    ...
    const generateAccountCode = async (
        ...
        try {
            const data = await apiFetch<{ data: string }>(
                `${baseURL}/api/hochartofaccount/generateaccountcode?parentAccountId=${parentId}`
            );
    ...
    const fetchChildAccounts = async (
        ...
        try {
            ...
            const data = await apiFetch<{ data: any[] }>(
                `${baseURL}/api/hochartofaccount/getchildaccountbylevel?${queryParent}accountLevel=${accountLevel}`
            );
    ...
    const GetUpdateAccountData = async (accountId: number | null) => {
        ...
        if (response.payload) {
            ...
            const requestBody = {
                ...
                regionId: accountData.regionId ?? null
            };
    ...
    const handleSubmit = async (
        ...
        AddChartofAccount: (payload: ChartOfAccountPayload) => unknown
    ): Promise<void> => {
        setIsSubmit(true);
        try {
            let {
            ...
            if (levelValue === 1) {
                ...
                try {
                    const res = await axios.get<{ data: { accountName: string } }>(
                        `${baseURL}/api/hochartofaccount/getaccountbyid?id=${natureId}`
                    );
            ...
            await dispatch(AddChartofAccount(body));
                ...
                <div className="content">
                    ...
                    <div className="d-md-flex d-block align-items-center justify-content-between mb-3">
                        <div className="my-auto mb-2">
                            <h3 className="page-title mb-1">Chart of Account</h3>
                            <nav>
                    ...
                    <div className="card">
                        <div className="card-header d-flex align-items-center justify-content-between flex-wrap pb-0">
                            <h4 className="mb-3">Head Office Chart of Account</h4>
                                                    ...
                                                    <div className="col-md-12">
                                                        <div className="mb-3">
                                                            ...
                                                            <CommonSelect3
                                                                className="select"
                                                                options={regionsList}
                                                                onChange={(option) => handleRegionCOA(option ? option.value : 0)}
                                                                value={searchRegionId ? regionsList.find(region => region.value === Number(searchRegionId)) : regionsList[0]}
                                                                loading={loading3}
                                                            />
                                                        </div>
                                                    </div>
                                    ...
                                    <div className="col-md-12">
                                        ...
                                        <div className="mb-3">
                                            ...
                                            <CommonSelect2
                                                ...
                                                defaultValue={userLevel === 2 ? coaentrylevel.find(item => item.value === 4) : coaentrylevel[0]}
                                            />
                                        </div>
                                        {mainAccount && (
                                            <div className="mb-3">
                                        ...
                                        {headOfAccount && (
                                            <div className="mb-3">
                                                <label className="form-label">Region (Opt)</label>
                                                <CommonSelect3
                                                    className="select"
                                                    options={regionsList}
                                                    onChange={(option) => handleChangeRegions(option ? option.value : 0)}
                                                    value={regionId ? regionsList.find(region => Number(region.value) === Number(regionId)) || null : regionsList[0]}
                                                    loading={loading3}
                                                />
                                            </div>
                                        )}
                                ...
                                <div className="modal-body">
                                    <div className="row">
                                        <div className="col-md-12">
                                            ...
                                            {updateAccountData?.accountLevel === 4 && (
                                                <div className="mb-3">
                                                    <label className="form-label">Region</label>
                                                    <CommonSelect3
                                                        className="select"
                                                        options={regionsList}
                                                        onChange={(option) => handleRegionChange(option ? option.value : 0)}
                                                        value={updateAccountData?.regionId ? regionsList.find(region => region.value === Number(updateAccountData?.regionId)) : regionsList[0]}
                                                        loading={loading3}
                                                    />
                                                </div>
                                            )}
